...
class DenseNet121(nn.Module):
    ...
    def __init__(self, nClasses):
        ...
        self.backbone = self.densenet121(pretrained=True)
        #num_ftrs = self.densenet121.classifier.in_features
        self.fc = nn.Linear(4096, nClasses)

    def densenet121(self, pretrained=False, small=1, **kwargs):
        ...
        if pretrained:
            ...
            model.load_state_dict(torch.load('D:\\workspace\\cygwin64\\home\\a811241\\chinese_ocr-master\\train\\pretrained_models\\densenet121.pth'), strict=False)
        return model

    def forward(self, x):
        #print(x.shape)
        feats = self.backbone(x)     # (32, 1024, 2, 16)
        #print(feats.shape)
        out = F.relu(feats)  # n c h w ---> n w  h c
