...
class DenseNet(nn.Module):
    def __init__(self, growthRate=8, depth=28, height = 32, nClasses=80, bottleneck=None):
        ...
        self.bn1 = nn.BatchNorm2d(nChannels, eps=1.1e-5)
        self.fc = nn.Linear(nChannels*height//8, nClasses)
...
class DenseCRNN(nn.Module):
    ...
    def __init__(self, nClasses=80, nh = 256):
        super(DenseCRNN, self).__init__()
        self.backbone = DenseNet(growthRate=8, depth=28, height = 32, nClasses=80, bottleneck=None)
        self.lstm = nn.Sequential(
            BidirectionalLSTM(768, nh, nh),      ##
            BidirectionalLSTM(nh, nh, nClasses))

    def forward(self, input):
        features = self.backbone(input) ##n w h*c
        features = features.permute(1, 0, 2) # w n h*c
        out = self.lstm(features)   #w n h*c
        out = out.permute(1, 0, 2)  #  n w h*c
