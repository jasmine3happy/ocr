...
def resize(image, width=None, height=None, minedge=None, inter=cv2.INTER_AREA):
...
def cal_rpn(imgsize, featuresize, scale, gtboxes):
    ...
    '''NEW'''
    regr_labels = copy.deepcopy(labels)
    regr_labels[anchor_max_overlaps > 0.5] = 1  #
    ...
    # __fg_sum += len(fg_index)
    # __bg_sum += len(bg_index)
    # print(len(fg_index), len(bg_index), __fg_sum/__it, __bg_sum/__it)
    # calculate bbox targets
    # debug here
    bbox_targets = bbox_transfrom(base_anchor, gtboxes[anchor_argmax_overlaps, :])
    # bbox_targets=[]

    return [labels, regr_labels, bbox_targets], base_anchor


def cal_rpn_paint(img, imgsize, featuresize, scale, gtboxes):
...
def ohem_cal_rpn(imgsize, featuresize, scale, gtboxes):
...
def cal_siderefine_targets(imgsize, featuresize, scale, gtboxes):
    # global __bg_sum, __fg_sum, __it
    # __it+=1
    # gen base anchor
    base_anchor = gen_anchor(featuresize, scale)
    ...
    labels = np.empty(base_anchor.shape[0])
    labels.fill(-1)
    targets = np.ones(base_anchor.shape[0])*1000

    thres = 0.5
    for [x1, y1, x2, y2] in gtboxes:
        t1 = np.where(0<=x1-base_anchor[:, 2]<32 and
                       np.minimum(base_anchor[:, 3], y2)-np.maximum(base_anchor[:, 1], y1)>=thres*(y2-y1))[0]
        t2 = np.where(0 <= base_anchor[:, 0]-x2 < 32 and
                      np.minimum(base_anchor[:, 3], y2) - np.maximum(base_anchor[:, 1], y1) >= thres * (y2 - y1))[0]
        labels[t1] = 1
        tmp = -((base_anchor[:, 2]+base_anchor[:, 0])/2-x1)/16
        tt1 = np.where(np.abs(tmp)<np.abs(targets))
        t1 = np.logical_and(t1, tt1)
        targets[t1] = tmp[t1]
        labels[t2] = 1
        tmp = -((base_anchor[:, 2]+base_anchor[:, 0])/2 -x2) / 16
        tt2 = np.where(np.abs(tmp) < np.abs(targets))
        t2 = np.logical_and(t2, tt2)
        targets[t2] = tmp[t2]
        ...
        for index in range(self.graph.shape[0]):
            if not self.graph[:, index].any() and self.graph[index, :].any():
                ...
                while self.graph[v, :].any():
                    v = np.where(self.graph[v, :])[0][0]  #若有多个，只连接了其中一个
...
class TextProposalGraphBuilder:
    ...
    def is_succession_node(self, index, succession_index):
        ...
        if self.scores[index] >= np.max(self.scores[precursors]): #why >= ??
            return True
        return False
