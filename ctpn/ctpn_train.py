...
if __name__ == '__main__':
    ...
    if os.path.exists(checkpoints_weight):
        ...
        resume_epoch = cc['epoch'] if 'epoch' in cc.keys() else 0

    params_to_uodate = model.parameters()
    optimizer = optim.SGD(params_to_uodate, lr=lr, momentum=0.9, weight_decay=0.0005)
    
    critetion_cls = RPN_CLS_Loss(device)
    critetion_regr = RPN_REGR_Loss(device, 9)
