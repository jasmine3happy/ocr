...
from ctpn_utils import cal_rpn, ohem_cal_rpn, cal_rpn_paint
...
class VOCDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        [cls, regr_cls, regr], _ = cal_rpn((h, w), (int(h / 16), int(w / 16)), 16, gtbox)

        m_img = img - IMAGE_MEAN

        regr = np.hstack([regr_cls.reshape(cls.shape[0], 1), regr])
...
class VOCOHEMDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #cal_rpn_paint(img, (h, w), (int(h / 16), int(w / 16)), 16, gtbox)
        [cls, regr], base_anchors = ohem_cal_rpn((h, w), (int(h / 16), int(w / 16)), 16, gtbox)
        ...
        return m_img, cls, regr, base_anchors
